...
const Settings = () => {
  const [_currentStep, _setCurrentStep] = useState('');
  ...
  const handleClick = () => {
    const handler = async () => {
      ...
      const checkChain = async (api: ApiPromise): Promise<boolean> => {
        try {
      ...
      //             if (section === 'supersig' && method === 'CallSubmitted') {
      //               console.log('Event data:', data.toJSON()); // Log the entire event data
      //               const callId = data[0].toString(); // Assuming the callId is the first argument of the event
      ...
      try {
        ...
        setStatus((prev) => ({ ...prev, errorMessage: 'An error occurred' }));
      }
    };
  ...
  return (
    <div>
      ...
      <button
        onClick={handleClick}
        style={{ margin: '5px', backgroundColor: '#000', color: '#FFFFFF', border: 'none', padding: '8px 15px 8px 15px', borderRadius: '4px', cursor: 'pointer' }}
      >
      Test Supersig
      </button>
