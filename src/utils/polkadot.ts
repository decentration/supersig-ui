...
export async function addDelayedProxy(
  api: ApiPromise,
  accountInjector: InjectedAccountWithMeta,
  delegate: string,
  proxyType: string,
  delay: number
) {
  ...
  if (signer) {
    ...
    const unsub = await signedTx.signAndSend(
      accountInjector.address,
      { signer },
      ({ status }) => {
        if (status.isInBlock) {
...
export async function announceProxy(
  api: ApiPromise,
  accountInjector: InjectedAccountWithMeta,
  real: string,
  callHash: string
