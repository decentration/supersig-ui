...
const Summary: React.FC<{ title: string; value: number }> = ({
  title,
  value,
}) => (
...
const DashboardButton: React.FC<{ icon: string; label: string }> = ({
  icon,
  label,
}) => (
  <button className='dashboard-button'>
    <span className='mr-2'>
      <FontAwesomeIcon
        icon={faPlus}
        style={{ marginRight: '7px', minWidth: '20px' }}
      />
    </span>
    {label}
  </button>
    ...
    const getTableData = async (api: ApiPromise | null) => {
      allAddress.map(async (address) => {
        ...
        setTableData((tableData) => [
          ...tableData,
          { address: address, balancesAll: balancesAll },
        ]);
      });
    };
    setbalance(api);
