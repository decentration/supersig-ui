...
import type {
  ...
  MembersList,
  ProposalState,
} from 'supersig-types/dist/interfaces/default';
...
function Address({
  ...
  const members = useCall<MembersList>(api.api.rpc.superSig.listMembers, [
    address,
  ]);
  const proposals = useCall<FetchListProposals>(
    api.api.rpc.superSig.listProposals,
    [address]
  );
  ...
  const getInfo = async () => {
    ...
    await Promise.all(
      (members?.toArray() || []).map(async (item: any) => {
        const balance = await api.api.derive.balances?.all(item[0]);
        const membalance = balance.freeBalance
          .add(balance.reservedBalance)
          .toString();
  ...
  const ProposalDetail = (): React.ReactElement => {
    const VoterComponent = ({
      ...
      voter: AccountId;
    }): React.ReactElement<{ voter: AccountId }> => {
      const balances = useCall<DeriveBalancesAll>(
        api.api.derive.balances?.all,
        [voter]
      );
      const voterbalance = balances
        ? balances?.freeBalance.add(balances?.reservedBalance).toString()
        : '0';
    ...
    const renderProposalInfo = useCallback(
      () =>
        ...
        proposals.proposals_info.map(
          (item: ProposalState, index: number): React.ReactNode => {
            let extrinsicCall: Call;

            // eslint-disable-next-line prefer-const
            extrinsicCall = api.api.createType(
              'Call',
              item.encoded_call.toString()
            );
            const { method, section } = api.api.registry.findMetaCall(
              extrinsicCall.callIndex
            );
            let call_id = u8aToHex(item.id.toU8a(), -1, false).toString();
            const call_data = u8aToHex(
              item.encoded_call.toU8a(),
              -1,
              false
            ).toString();
            ...
            const detailslink =
              '#/organisations/decode/0x08016d6f646c69642f7375736967' +
              '00000000000000000000000000000000000000' +
              call_data;
            const approvelink =
              '#/organisations/create/0x2a026d6f646c69642f7375736967' +
              nonce.slice(26, 28) +
              '00000000000000000000000000000000000000' +
              call_id;
                ...
                >
                  ...
                  >
                    <Expander
                      summary={t<string>(
                        'Voters(' +
                          item.voters.length.toString() +
                          '/' +
                          proposals.no_of_members.toString() +
                          ')'
                      )}
                    >
                      {item.voters.map((voterId: AccountId, i: number) => {
                        return <VoterComponent key={i} voter={voterId} />;
                      })}
                    </Expander>
                  </div>
                  <div
  ...
  const BalanceDetail = (): React.ReactElement => {
    const renderMembers = useCallback(
      () =>
        memberAccounts &&
        memberAccounts.map(
          (item: IMemberItem, index: number): React.ReactNode => (
            <div key={index} style={{ display: '-webkit-box' }}>
              ...
              <div>
                ...
                <FormatBalance className='result' value={item.balance} />
              </div>
            </div>
          )
        ),
    ...
    return (
      <ExpanderScroll
        ...
        summary={
          <>
            <FormatBalance className='result' value={balancesSum} /> (
    ...
    <>
      <tr className={`${className}${isExpanded ? ' noBorder' : ''}`}>
        ...
        <td className='number media--1500'>
          {balancesAll?.accountNonce.gt(BN_ZERO) &&
            formatNumber(balancesAll.accountNonce)}
        </td>
        <td className='number'>
          <AddressInfo
            address={address}
        ...
        <td className='fast-actions-addresses'>
          <div className='fast-actions-row'>
            {isFunction(api.api.tx.balances?.transfer) && (
              <a
                href={`#/organisations/create/0x08016d6f646c69642f7375736967${nonce.slice(
                  26,
                  28
                )}00000000000000000000000000000000000000`}
              >
